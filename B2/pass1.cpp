...
void AssemblerPass1::performPass() {
    ...
    for (auto &instruction : instruction_tokens) {
        ...
        // ORIGIN
            ...
            if (checkSymbolOffsetExpr(op1)) {
                ...
                locationCounter = symbolTable.find(op1) ? 
                                  symbolTable.getSymbolAddress(op1) : std::stoi(op1);
                icOutputStream << " (" << mnemonic.getClass() << "," << mnemonic.opCode << ") (C," 
                               << locationCounter << ")" << LBR; 
            }
        }
        ...
        // Declarative Statements
            ...
            locationCounter += (mnemonic_string == "DS") ? std::stoi(op1) : 1;
        }
    }
...
bool AssemblerPass1::checkSymbolOffsetExpr(const string& expr) {
    return (expr.find("+") != string::npos) || (expr.find("-") != string::npos); 
}

int AssemblerPass1::evaluateSymbolOffsetExpr(const string& expr, string& symbolName) {
...
string AssemblerPass1::extractLiteralValue(const string& literalToken) {
...
void AssemblerPass1::printIC() {
    string sourceContents = readTextFile(icFilepath);
    cout << sourceContents << LBR; 
}

string AssemblerPass1::readTextFile(const string& filepath) {
...
vector<string> AssemblerPass1::readLines(const string& contents) {
    vector<string> lines;
...
void AssemblerPass1::saveTables(const SymbolTable& sym, const LiteralTable& lit) {
